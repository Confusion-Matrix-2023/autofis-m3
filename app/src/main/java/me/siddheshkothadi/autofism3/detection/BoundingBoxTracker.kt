package me.siddheshkothadi.autofism3.detection
...
import me.siddheshkothadi.autofism3.detection.env.BorderedText
import me.siddheshkothadi.autofism3.detection.env.ImageUtils
import me.siddheshkothadi.autofism3.detection.env.Logger
import me.siddheshkothadi.autofism3.detection.tflite.Classifier.Recognition
import java.util.*
import kotlin.math.min

/** A tracker that handles non-max suppression and matches existing objects to new detections.  */
class BoundingBoxTracker(context: Context) {
    ...
    fun draw(canvas: Canvas) {
        ...
        for (recognition in trackedObjects) {
            ...
            val cornerSize: Float = min(trackedPos.width(), trackedPos.height()) / 8.0f
    ...
    private fun processResults(results: List<Recognition>) {
        ...
        for (potential in rectsToTrack) {
            ...
            trackedRecognition.color =
                BoundingBoxTracker.COLORS.get(
                    potential.second.detectedClass % COLORS.size
                )
